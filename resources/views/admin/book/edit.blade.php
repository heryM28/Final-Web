...
<div class="max-w-3xl mx-auto p-6 bg-white shadow-md rounded-lg mt-10">
    <h1 class="text-xl font-bold text-gray-700 mb-6 text-center">Edit Buku</h1>
    ...
    <form action="{{ route('admin.book.update', $book->id) }}" method="POST" class="space-y-4">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
                ...
                <input type="number" name="publication_year" id="publication_year" min="1000" max="{{ date('Y') }}" value="{{ $book->publication_year }}" required
                    class="w-full border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500">
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                class="w-full border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500"
                placeholder="Opsional">{{ $book->description }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
                ...
                <input type="number" name="stock" id="stock" required min="0" value="{{ $book->stock }}"
                    class="w-full border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500">
            </div>
            ...
            <div>
                ...
                <select name="is_available" id="is_available" required
                    class="w-full border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500">
                    <option value="1" {{ $book->is_available ? 'selected' : '' }}>Ya</option>
                    <option value="0" {{ !$book->is_available ? 'selected' : '' }}>Tidak</option>
                </select>
            </div>
        </div>
